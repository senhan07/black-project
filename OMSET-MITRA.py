...
for sales in sales_array:
    ...
    total = total[total['Customer'].str.match(sales, na=False)]
    total_total = total['Total'].sum()

    pushon = pushon[pushon['Customer'].str.match(sales, na=False)]
    pushon = pushon[pushon['Brand'].str.match('PUSH O|LUMMEN', na=False)]
    total_pushon = pushon['Total'].sum()

    cahaya = cahaya[cahaya['Customer'].str.match(sales, na=False)]
    cahaya = cahaya[cahaya['Brand'].str.match('CAHAYA|SZMR|PANCAR|HAKAMI|FROZEN', na=False)]
    total_cahaya = cahaya['Total'].sum()

    dexicon = dexicon[dexicon['Customer'].str.match(sales, na=False)]
    dexicon = dexicon[dexicon['Brand'].str.match('CROWN|DEXICO|ENJE|ENJE O|HERCUL|KINGWO|KSRAN|LEXICO|NJ|UNITED|YUKI|VENUS|VENUSI|VENUSIA|DEXICON|GRONCO|KINGWON|LEXICON|LUXURI', na=False)]
